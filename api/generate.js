// Serverless-функция для Vercel, использующая Hugging Face Inference API (Stable Diffusion 2.1).
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Безопасные рамки для SD 2.1 (обычно 512 или 768 стабильнее на бесплатных лимитах)
    ...
    // Модель: стабильная и популярная
    const model = "stabilityai/stable-diffusion-2-1";

    // Запрос к Inference API.
    // ВАЖНО: ответ — бинарное изображение (не JSON), поэтому читаем как arrayBuffer.
